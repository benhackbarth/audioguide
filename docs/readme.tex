...
\newcommand{\ag}{audioguide1.2\xspace}
\newcommand{\Ag}{Audioguide1.2\xspace}
...
\item[pysdif (AudioGuide comes with this module prebuilt)] To compile install download the \href{http://sourceforge.net/projects/sdif/files/sdif/}{sdif library} and configure, make and install.  Then download my patched version of the \href{http://www.benhackbarth.com/audioGuide/release/pysdif-0.1.4-patch.zip}{pysdif module}.  `cd' into the directory when unzipped and run: `python2.7 setup.py install'.  You're now setup to use pysdif in python2.7.
\item[numpy (python $>$= 2.7 has numpy pre installed)] Numpy is a numerical computation module.  Upgrading to the latest python2.7 should get you there.  If you don't have it, you can download the source code or a binary installer \href{http://sourceforge.net/projects/numpy/files/NumPy/}{here}.
\item[matlibplot (very optional)] Install this python module to enable graphing of descriptors and soundfile segmentation.  Get it from \href{http://matplotlib.org/downloads.html}{here}.
\item[supervp (very optional)] Used to stretch a target before concatenating.
\end{description}

  
\section{Really Quick Start}
...
The SEARCH variable specifies how \ag picks corpus segments to match target segments.  The idea here is make a \emph{very} flexible searching structure where the user can create multiple search passes based on different descriptor criteria.
...
Where the search\_type string may be among the following methods:
\begin{description}
...
\section{Descriptors and the d() object}
Use the d() object for parameterizing a descriptor in an spass() object.  The d() object takes 1 argument -- the name of the desired descriptor --  and then several optional keywords arguments, detailed below.
...
\textbf{descriptor name} a string of the name of the desired descriptor.  Note that most (but not all) descriptors have both a time varying version as well as a power-weighted averaged version (`-seg').  Also note that any time varying descriptor can take the appendage `-delta' for the first order difference and `-deltadelta' for the second order difference.  `-delta-seg' and `-deltadelta-seg' are also possible for averaged delta descriptors.  Thus `centroid-delta-seg' is a power-weighted average of the first order difference of time varying centroids.  All possible descriptors are listed in the following table.

\rowcolors{2}{gray!25}{white}
   \small
    \begin{longtable}{|p{5cm}|p{4.5cm}|p{3.5cm}|} \hline
                                                & Averaged Descriptor Name                & Time Varying Descriptor Name \\ \hline
    duration                                              & dur-seg                   &                   \\
    effective duration                                              & effDur-seg\footnote{effDur-seg ignores silences whereas dur-seg does not}                   &                \\
    peak amplitude time                         & peakTime-seg                   &                \\
    midi pitch from filename                         & MIDIPitch-seg\footnote{myfileC4.wav=60, basooonFs2.wav=42, HA\_ES\_mu\_p\_Ab6.wav=92, etc.}                   &             \\
    amplitude                                             & power-seg, power-mean-seg\footnote{power-seg is a segment's peak amplitude.  power-mean-seg is the mean of a segment's time varying amplitudes.}                    & power                  \\
    ...
    fundamental frequency                                 & f0-seg\footnote{f0-seg is the median f0, not a power weighted average.  Thus f0-seg will be a pitch actually present in the f0 time varying array.}                       & f0                     \\
    zero crossings                                        & zeroCross-seg                & zeroCross              \\
    combination of 12 mel frequency cepstral coefficients & mfccs-seg                    & mfccs                  \\
     mel frequency cepstral coefficient 1 ... 12          & mfcc0-seg ... mfcc11-seg     & mfcc0 ... mfcc11       \\
    ...
    perceptual spectral variation       & perceptualvariation-seg           & perceptualvariation      \\ \hline
