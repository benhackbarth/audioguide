...
class output:
	...
	###########################################
	def write(self, autolaunchbool, verbose=True, rpp_header='REAPER_PROJECT 0.1 "6.11/x64" 1591355987', playrate_change_duration=True, playrate_preset=-1):
		...
		for trackname, trackitems, trackorigin in self.tracks:
			...
			for d in trackitems:
				...
				track_str += '''     <ITEM\n      POSITION  %f\n      NAME %s\n      LENGTH %f\n      SOFFS %f\n      VOLPAN %f 0.0 1.0 -1.0 \n      FADEIN 1 %f 0.0\n      FADEOUT 1 %f 0.0\n      %s      <SOURCE WAVE\n        FILE "%s"\n      >\n     >\n'''%(d['time'], d['name'], d['orig_duration'], d['skip'], d['ampscale'], d['fadein'], d['fadeout'], playrate_str, d['file'])
