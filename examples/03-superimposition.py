...
##                                                                            ##
## python3 agConcatenate.py examples/03-superimposition.py                    ##
##                                                                            ##
################################################################################
...
#### minFrame, maxFrame - tell Ag how many corpus segments to pick for each analysis frame of each target segment.  frame speed is taken from DESCRIPTOR_HOP_SIZE_SEC, by default 0.01024 seconds.  by default there is no minimum or maximum and Ag will pick / not pick corpus segments according to a subtractive amplitude model.

#### minOverlap, maxOverlap - tell Ag how many corpus segments must be playing during each target frame.  unlike minSegment/maxSegment/minFrame/maxFrame, overlap takes corpus sounds' duration into account.  by default there is no minimum or maximum.
...
#SUPERIMPOSE = si(maxFrame=1) # ag is restricted to picking one corpus sound for each frame of a target segment.  this limits the number of sounds that may be selected at any single moment to 1.

#SUPERIMPOSE = si(minFrame=2, maxFrame=4) # ag must pick between 2-4 corpus sounds for each frame of each target segment.  every frame must include two new corpus sounds up to a maximum of four.  this will result in a lot of segments!

#SUPERIMPOSE = si(minFrame=4, maxSegment=4) # ag will pick 4 sounds in the first frame, then nothing for the rest of each target segments' duration.  like a temporally aligned "chord" of four elements.  due to the internal superimposition logic, minSegment/maxSegment always overrides minFrame/maxFrame.
